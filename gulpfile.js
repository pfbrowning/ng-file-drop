...
const config = {
  libraryName: '@browninglogic/ng-file-drop',
  unscopedLibraryName: 'ng-file-drop',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('build:demo', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /ng-file-drop/ --deploy-url /ng-file-drop/`, { cwd: `${config.demoDir}`});
});
...
gulp.task('build:demo-ssr', () => {
  return execDemoCmd(`build --preserve-symlinks --prod`, { cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run @browninglogic/ng-file-drop-demo:server`, { cwd: `${config.demoDir}` }))
